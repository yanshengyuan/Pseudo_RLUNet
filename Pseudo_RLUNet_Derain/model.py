...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        # rEncoder
        self.rencoderlayer_0 = BasicUformerLayer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[0],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rdowsample_0 = dowsample(embed_dim*2, embed_dim*2)
        self.rencoderlayer_1 = BasicUformerLayer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(img_size // 2,
                                                img_size // 2),
                            depth=depths[1],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rdowsample_1 = dowsample(embed_dim*4, embed_dim*4)
        self.rencoderlayer_2 = BasicUformerLayer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[2],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rdowsample_2 = dowsample(embed_dim*8, embed_dim*8)
        self.rencoderlayer_3 = BasicUformerLayer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[3],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rdowsample_3 = dowsample(embed_dim*16, embed_dim*16)

        # rBottleneck
        self.rconv = BasicUformerLayer(dim=embed_dim*16,
                            output_dim=embed_dim*16,
                            input_resolution=(img_size // (2 ** 4),
                                                img_size // (2 ** 4)),
                            depth=depths[4],
        ...
        # rDecoder
        self.rupsample_0 = upsample(embed_dim*16, embed_dim*8)
        self.rdecoderlayer_0 = BasicUformerLayer(dim=embed_dim*8,
                            output_dim=embed_dim*8,
                            input_resolution=(img_size // (2 ** 3),
                                                img_size // (2 ** 3)),
                            depth=depths[5],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rupsample_1 = upsample(embed_dim*8, embed_dim*4)
        self.rdecoderlayer_1 = BasicUformerLayer(dim=embed_dim*4,
                            output_dim=embed_dim*4,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[6],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rupsample_2 = upsample(embed_dim*4, embed_dim*2)
        self.rdecoderlayer_2 = BasicUformerLayer(dim=embed_dim*2,
                            output_dim=embed_dim*2,
                            input_resolution=(img_size // 2,
                                                img_size // 2),
                            depth=depths[7],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.rupsample_3 = upsample(embed_dim*2, embed_dim)
        self.rdecoderlayer_3 = BasicUformerLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
    ...
    def forward(self, x):
        ...
        #Recurrent process----------------------------------------------------
        #rEncoder
        y=torch.cat([y,deconv3],-1)
        conv0 = self.rencoderlayer_0(y)
        
        pool0 = self.rdowsample_0(conv0)
        conv1 = torch.cat([pool0,deconv2],-1)
        conv1 = self.rencoderlayer_1(conv1)
        
        pool1 = self.rdowsample_1(conv1)
        conv2 = torch.cat([pool1,deconv1],-1)
        conv2 = self.rencoderlayer_2(conv2)
        
        pool2 = self.rdowsample_2(conv2)
        conv3 = torch.cat([pool2,deconv0],-1)
        conv3 = self.rencoderlayer_3(conv3)
        
        pool3 = self.rdowsample_3(conv3)

        # Bottleneck
        conv4 = self.rconv(pool3)

        #rDecoder
        up0 = self.rupsample_0(conv4)
        deconv0 = self.rdecoderlayer_0(up0)
        
        up1 = self.rupsample_1(deconv0)
        deconv1 = self.rdecoderlayer_1(up1)

        up2 = self.rupsample_2(deconv1)
        deconv2 = self.rdecoderlayer_2(up2)

        up3 = self.rupsample_3(deconv2)
        deconv3 = self.rdecoderlayer_3(up3)   
        #Recurrent Over--------------------------------------------------------------
